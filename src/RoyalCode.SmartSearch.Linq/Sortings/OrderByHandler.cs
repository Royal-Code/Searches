    ...
    where TModel : class
{
    ...
    public OrderByBuilder<TModel> Handle(OrderByBuilder<TModel> builder) => builder.Add(expression);
}
...
internal static class OrderByHandler
{
        ...
        where TModel : class
    {
        ...
        var handler = handlerType.GetConstructors().First().Invoke([expression]);
